...
/**
 * The static tutor prompt. FROZEN BYTES per deploy — no interpolation.
 * (Prompt caching on OpenRouter is provider-dependent; keeping the stable
 * prefix first costs nothing and pays off wherever caching applies.)
 */
...
System messages may report elapsed session time against the student's planned length. At natural stopping points after such a message, suggest a short break in one sentence (call record_break_suggestion when you do). Never interrupt the middle of a problem for a break.
...
/** Query interface — injectable so the determinism test runs without a DB. */
export interface Db {
  // deno-lint-ignore no-explicit-any
  q: (text: string, params?: unknown[]) => Promise<any[]>;
  // deno-lint-ignore no-explicit-any
  one: (text: string, params?: unknown[]) => Promise<any | null>;
}

/**
 * Per-user course context. MUST BE DETERMINISTIC for identical DB state
 * (sorted queries, stable serialization, no timestamps).
 */
export async function buildCourseContext(
  ...
  const profile = await db.one(
    "select display_name, grade_level, program from profiles where id = $1",
    [userId],
  );
  ...
  if (courseId) {
    const course = await db.one(
      "select name, subject, grade_level, instructor, term, curriculum, context_summary from courses where id = $1 and user_id = $2",
      [courseId, userId],
    );
    ...
    if (course) {
      ...
      if (course.curriculum) parts.push("## Curriculum\n" + course.curriculum);
      if (course.context_summary) {
        parts.push("## Course content summary\n" + course.context_summary);
      }

      const materials = await db.q(
        "select id, title, kind, summary, topics from materials where course_id = $1 and user_id = $2 and status = 'ready' order by title asc, id asc",
        [courseId, userId],
      );
      ...
      const grades = await db.q(
        "select title, score, max_score, topics, graded_at, feedback from grades where course_id = $1 and user_id = $2 order by graded_at desc, id asc limit 10",
        [courseId, userId],
      );
      if (grades.length > 0) {
        ...
        for (const g of grades) {
          ...
          const day = g.graded_at instanceof Date
            ? g.graded_at.toISOString().slice(0, 10)
            : String(g.graded_at).slice(0, 10);
          parts.push(
            `- ${day} "${g.title}": ${g.score}/${g.max_score} (${pct}%)` +
              (topics ? ` [topics: ${topics}]` : "") +
      ...
      const assignments = await db.q(
        "select title, due_at, status, complexity, estimated_minutes from assignments where course_id = $1 and user_id = $2 and status <> 'done' order by due_at asc nulls last, id asc limit 10",
        [courseId, userId],
      );
      if (assignments.length > 0) {
        ...
        for (const a of assignments) {
          const due = a.due_at instanceof Date
            ? a.due_at.toISOString().slice(0, 10)
            : a.due_at
...
/** JSON with sorted keys — deterministic serialization. */
